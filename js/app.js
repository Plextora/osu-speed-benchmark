...
/* don't we all suck ass at variable naming? 
at one point even after i learned that you shouldn't make var names "a"
but i just did it all the letters in alphabetical order because i was so frustrated */
...
function beginTest() {
    ...
    if (clickLimit < 3)
    {
        alert("Please enter a value larger than 3 dumbo");
        testrunning = false;
        return false;
    }
    clickTimes.length = 0;
    ...
    key1 = $('#key1').val();
    key2 = $('#key2').val();
    $("div#status").html("The benchmark is ready! Start streaming to begin");
    $("div#Result").html("\
            ...
            if ((String.fromCharCode(event.which) == key1) || (String.fromCharCode(event.which) == key2))
            {
                switch (beginTime)
                {
                    case -1:
                        ...
                        $("div#status").html("The benchmark has started and is currently running");
                    default:
                ...
                if (clickTimes.length == clickLimit)
                {
                    ...
                    $("div#status").html("Timer starts when you make your first click.");
                    return;
                }
            }
        }
